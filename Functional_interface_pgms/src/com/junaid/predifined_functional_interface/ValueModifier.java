...
public class ValueModifier {
	...
	public static void main(String[] args) {
		int num = Integer.parseInt(IO.readln("Enter the NO: "));
		IO.println("Original Value: " + num);
		Consumer<Integer> doubleValue = (doble) -> {
			IO.println("After Doubling the Value : " + (doble * 2));
		};

		modifyValue(num, doubleValue);

		Consumer<Integer> increamentValue = (inc) -> {
			IO.println("After Increamenting the value by 3 : " + (inc + 3));
		};

		modifyValue(num, increamentValue);
		Consumer<Integer> squareValue = (square) -> {
			IO.println("After Squaring the Value : " + (square * square));
		};
