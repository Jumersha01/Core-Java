...
class Child extends Parent {
	@Override
	public void loadingClass(String className) throws ClassNotFoundException {
		if (className.isBlank() || className.isEmpty() || className.equals("null")) {
			throw new IllegalArgumentException();
		} else {
			super.loadingClass(className);
			IO.println("Class " + className + " loaded succesfully.");
		}
	}
}
	...
	public static void main(String[] args) {
		try {
			...
			if (e instanceof ClassNotFoundException) {
				IO.println("Error Class Not Found: " + args[0]);
			} else {
				IO.print("Provide class name: it cant be null || blank || Empty");
			}
		}
	}
}
