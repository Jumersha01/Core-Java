...
class CarPunctureException extends Exception {
	CarPunctureException(String errorMessage) {
		super(errorMessage);
	}
}

class CarHeatException extends Exception {
	CarHeatException(String errorMessage) {
		super(errorMessage);
	}
}
...
class CarTest {
	...
	public static void CarStoppedException(String stop) throws CarStoppedException {
		if (stop.equalsIgnoreCase("Stop")) {
			throw new CarStoppedException("Your car is stoped");
		} else {
			IO.println("Car not stall");
		}
	}

	public static void CarPunctureException(String puncture) throws CarPunctureException {
		if (puncture.equalsIgnoreCase("puncture")) {
			throw new CarPunctureException("Your car got punctured");
		} else {
			IO.println("Car not Stall");
		}
	}

	public static void CarHeatException(int heat) throws CarHeatException {

		if (heat>50) {
			throw new CarHeatException("Your card got heat");
		} else {
			IO.println("Car not stall");
		}
	}
...
public class CarUser {
	...
	public static void main(String[] args) {
		try {
			...
			CarTest.CarStoppedException("stop");
			CarTest.CarPunctureException("puncture");
			CarTest.CarHeatException(55);
		} catch (Exception e) {
			IO.println(e);
		}
	}
