...
class FinancialTransaction {
	public void processTransaction(double amount, long accountNO) {
		if (amount <= 0) {
			...
			IO.println("Processing transaction...\n" + "Transaction successful: Amount " + amount
					+ " transferred to account " + accountNO);
		}
	}
}
...
public class FinancialTransactionTester {
	...
	public static void main(String[] args) {
		try {
			...
			try {
				amount = Double.parseDouble(IO.readln("Enter the Amount: "));
				accoutNO = Long.parseLong(IO.readln("Enter Account NO: "));

			} catch (NumberFormatException e) {
				IO.println("Invalid input: Please enter a valid number for transaction amount.");
			}
			FinancialTransaction transaction = new FinancialTransaction();
			transaction.processTransaction(amount, accoutNO);
